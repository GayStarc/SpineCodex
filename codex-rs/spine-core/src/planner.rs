...
pub(crate) struct SamplingPlanner {
    ...
    epoch: ContextEpoch,
    source: SourceLedger,
    epoch_start_boundary: BoundaryId,
...
impl SamplingPlanner {
    pub fn new(
        ...
        let epoch_start_boundary = BoundaryId::new(thread.clone(), epoch, 0);
        Ok(Self {
            source: SourceLedger::new(thread.clone(), epoch).map_err(PlannerError::Source)?,
            epoch_start_boundary,
    ...
    pub fn continue_in_namespace(&mut self, thread: ThreadNamespace) -> Result<(), PlannerError> {
        ...
        self.source
            .continue_in_namespace(thread.clone(), self.committed_source_cells)
    ...
    pub fn begin_sampling(
        ...
        if attempt_id.thread() != &self.thread {
            return Err(PlannerError::IdentityScopeMismatch);
        }
        let pre_boundary = self.current_boundary();
        let attempt = SamplingAttempt::new(attempt_id.clone(), self.epoch, pre_boundary)
    ...
    pub(crate) fn prepare_sampling_with_input_tokens(
        ...
        let fact_sources = reduce_sampling_delta(
            &snapshot,
            self.committed_source_cells,
            RawBoundary(post_boundary.ordinal()),
            &mut parser,
            &mut compiler,
            &ordered_facts,
    ...
    pub fn install_prepared(
        ...
        if prepared.record.attempt_id.thread() != &self.thread
            || prepared.record.epoch != self.epoch
        {
            return Err(PlannerError::IdentityScopeMismatch);
        }
    ...
    pub(crate) fn prepare_compact(
        ...
        candidate
            ...
            .map_err(PlannerError::Source)?;
        candidate.epoch = barrier.next_epoch;
        candidate.epoch_start_boundary = candidate.source.current_boundary();
    ...
    pub(crate) fn from_replay_state(
        ...
        Ok(Self {
            ...
            epoch: state.epoch,
            source: state.source,
            epoch_start_boundary: state.epoch_start_boundary,
            parser: state.parser,
            compiler,
    ...
    fn validate_commit_scope(
        ...
        if sealed.attempt().epoch() != self.epoch
            || sealed.attempt().attempt_id().thread() != &self.thread
            || sealed.attempt().pre_boundary().thread() != &self.thread
            || post_boundary.thread() != &self.thread
            || post_boundary.epoch() != self.epoch
            || commit_id.thread() != &self.thread
        {
            return Err(PlannerError::IdentityScopeMismatch);
        }
        ...
        for fact in sealed.facts() {
            if fact.execution_id.thread() != &self.thread {
                return Err(PlannerError::IdentityScopeMismatch);
            }
