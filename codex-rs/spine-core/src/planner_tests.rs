...
#[test]
fn sampling_planner_memory_slots_preserve_close_and_atomic_next() {
    let mut planner =
        SamplingPlanner::new(namespace(), ContextEpoch::ZERO, config()).expect("planner");
    planner
    ...
    finish_sampling(&mut planner, &open, 3, "commit-open-memory");
    ...
    let output = finish_sampling(&mut planner, &next, 6, "commit-next-memory");
