    ...
    E: std::error::Error,
{
    ...
    match item {
        ...
        ContextItem::ToolCall(group) => {
            let group_cells = source
                .iter()
                .filter(|cell| {
                    !used.contains(&cell.id())
                        && group.start <= cell.character().boundary()
                        && cell.character().boundary() <= group.end
                        && cell_belongs_to_group(cell, group)
                })
                .cloned()
                .collect::<Vec<_>>();
            ...
            for cell in group_cells {
                used.insert(cell.id());
                let labels = tool_cell_labels(&cell, group, trim, spawn_enabled);
                target.push(cell.with_labels(labels));
            }
        }
...
fn cell_belongs_to_group(cell: &ParseCell, group: &ToolCallGroup) -> bool {
    match cell.character() {
        SpineChar::Message(message) => group
            .leading_assistant_messages
            .iter()
            .any(|leading| leading.boundary == message.boundary),
        SpineChar::ToolRequest(request) => group
            .calls
            .iter()
            .any(|call| call.call_id == request.call_id),
        SpineChar::ToolResponse(response) => group
            .calls
            .iter()
...
fn tool_cell_labels(
    ...
    if spawn_enabled
        && let Some(call) = group
            .calls
            .iter()
            .find(|call| call.call_id == response.call_id && call.name == "spine.spawn")
    {
        ...
        labels.push(ContextLabel::SpawnOutput {
            succeeded: !conflicting && call.outcome == Some(ToolOutcome::Succeeded),
        });
    }
    labels
}
