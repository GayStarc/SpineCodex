...
impl ContextPlanBuilder<'_> {
    fn push_item(&mut self, item: &ContextItem) -> Result<(), PlannerError> {
        ...
        match item {
            ...
            ContextItem::ToolCall(group) => self.push_group(group)?,
    ...
    fn push_group(&mut self, group: &ToolCallGroup) -> Result<(), PlannerError> {
        let source_cells = self
            ...
            .filter(|cell| {
                !self.used_source.contains(&cell.id)
                    && group.start.0 <= cell.boundary.ordinal()
                    && cell.boundary.ordinal() <= group.end.0
                    && source_cell_belongs_to_group(cell, group)
            })
            .cloned()
            .collect::<Vec<_>>();
        if source_cells.is_empty() {
            return Err(PlannerError::MissingSourceBoundary(group.start));
        }
        for cell in source_cells {
            let labels = source_group_labels(&cell, group, self.trim, self.spawn_enabled);
            self.push_source(&cell, labels);
        }
        Ok(())
    }
...
fn source_cell_belongs_to_group(cell: &SourceCell, group: &ToolCallGroup) -> bool {
    match &cell.payload {
        SourceCellPayload::Message(message) | SourceCellPayload::TurnAborted(message) => group
            .leading_assistant_messages
            .iter()
            .any(|leading| leading.boundary == message.boundary),
        SourceCellPayload::ToolRequest { call_id, .. }
        | SourceCellPayload::ToolResponse { call_id, .. } => {
            group.calls.iter().any(|call| &call.call_id == call_id)
        }
        SourceCellPayload::Opaque | SourceCellPayload::Synthetic(_) => false,
    }
}

fn source_group_labels(
    cell: &SourceCell,
    group: &ToolCallGroup,
    trim: Option<&TrimProjection>,
    spawn_enabled: bool,
) -> Vec<ContextLabel> {
    let SourceCellPayload::ToolResponse {
        call_id, outcome, ..
    } = &cell.payload
    else {
        return Vec::new();
    ...
    if spawn_enabled
        && group
            .calls
            .iter()
            .any(|call| call.call_id == *call_id && call.name == "spine.spawn")
    {
        ...
        labels.push(ContextLabel::SpawnOutput {
            succeeded: !conflicting && *outcome == ToolOutcome::Succeeded,
        });
    }
    labels
}
