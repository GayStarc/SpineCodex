...
impl ArchiveLedger {
    ...
    ) -> Result<ArchiveObservation, ArchiveError> {
        if self.corrupted {
            return Err(ArchiveError::LedgerCorrupted);
        }
        record.validate()?;
        let Some(commit_id) = record.commit_id() else {
            return Ok(ArchiveObservation::Applied);
        };
        match self.commits.get(commit_id) {
            Some(digest) if digest == record.record_digest() => Ok(ArchiveObservation::Duplicate),
            Some(_) => {
                self.corrupted = true;
            ...
            None => {
                self.commits
                    .insert(commit_id.clone(), record.record_digest().clone());
                Ok(ArchiveObservation::Applied)
            }
        }
    }
