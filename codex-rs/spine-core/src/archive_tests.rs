...
use crate::archive::ArchiveObservation;
...
#[test]
fn sampling_archive_duplicate_is_idempotent_and_conflict_poisoned() {
    let mut ledger = ArchiveLedger::default();
    let record = commit("scope");
    assert_eq!(
        ledger.observe(&record).expect("first commit"),
        ArchiveObservation::Applied
    );
    assert_eq!(
        ledger.observe(&record).expect("duplicate commit"),
        ArchiveObservation::Duplicate
    );
    assert!(matches!(
        ledger.observe(&commit("different scope")),
        Err(ArchiveError::ConflictingCommit { .. })
    ));
    assert!(ledger.is_corrupted());
    assert!(matches!(
        ledger.observe(&record),
        Err(ArchiveError::LedgerCorrupted)
    ));
}
