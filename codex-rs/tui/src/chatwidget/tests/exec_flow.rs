...
#[tokio::test]
async fn spine_tree_snapshot_does_not_displace_a_running_exec() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.on_task_started();

    let begin = begin_exec(&mut chat, "call-running", "printf done");
    assert!(drain_insert_history(&mut rx).is_empty());
    let active_before = active_blob(&chat);

    let mut state = crate::history_cell::SpineTreeViewState::new(/*animations_enabled*/ true);
    state.apply_tree_update(codex_app_server_protocol::SpineTreeUpdatedNotification {
        thread_id: "thread-1".to_string(),
        turn_id: "turn-1".to_string(),
        snapshot_seq: 1,
        ...
        nodes: vec![codex_app_server_protocol::SpineTreeNode {
            ...
            summary: Some("inspect while exec is running".to_string()),
            memory_summary: None,
            start: 0,
            end: None,
    ...
    let presentation = crate::history_cell::SpineTreeWorkingPresentation::current_turn(
        std::time::Instant::now(),
        std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true)),
    );
    let cell = state
        .working_snapshot_cell(presentation)
        .expect("tree snapshot cell");

    chat.show_spine_tree_snapshot(cell);

    assert_eq!(
        active_blob(&chat),
        active_before,
        "the command snapshot must not replace the running exec cell"
    );
    let mut snapshot_cell = match rx.try_recv() {
        Ok(AppEvent::InsertHistoryCell(cell)) => cell,
        other => panic!("expected command snapshot history event, got {other:?}"),
    };
    assert!(
        lines_to_single_string(&snapshot_cell.display_lines(/*width*/ 80))
            .contains("inspect while exec is running")
    );
    assert!(snapshot_cell.transcript_animation_tick().is_some());
    snapshot_cell.finalize_for_history();
    assert_eq!(snapshot_cell.transcript_animation_tick(), None);

    end_exec(&mut chat, begin, "done\n", "", /*exit_code*/ 0);

    let completed_exec = drain_insert_history(&mut rx);
    assert_eq!(
        completed_exec.len(),
        1,
        "the running exec should complete into exactly one history cell"
    );
    let completed_exec = lines_to_single_string(&completed_exec[0]);
    assert!(completed_exec.contains("printf done"));
    assert!(completed_exec.contains("done"));
    assert!(
        chat.transcript.active_cell.is_none(),
        "the completed exec must not leave a stale running cell"
    );
}
