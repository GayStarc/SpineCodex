...
{inherited_model_guidance}
Use this ordinary multi-agent tool for one concrete, bounded, self-contained subtask that can run independently while you continue useful local work. It supports follow-up messages and parent supervision. If the current work instead has at least two substantial, mutually independent branches and `spine.spawn` is available, prefer one `spine.spawn` call for that fission/join; otherwise continue locally.
