...
#[cfg(test)]
pub(crate) use session::SpineSamplingAttemptGuard;
...
impl CodexSpineCoordinator {
    ...
    pub(crate) fn finish_canonical_sampling_with_input_tokens(
        ...
        let transition = encode_spine_transition(&SamplingArchiveRecord::SamplingCommit(
    ...
    pub(crate) fn install_canonical_sampling(
        ...
        let output = self.runtime.install_prepared(commit.prepared)?;
        let context = prepare_codex_context_plan(
            &output.plan,
    ...
    pub(crate) fn publish_canonical_compact(&mut self, projection: &SpineProjection) {
        self.observer.publish_committed(
            projection,
            &self.usage_samples,
            None,
    ...
    pub(crate) fn compact_live(
        ...
        let projection = self.runtime.compact(barrier)?;
        ...
        self.publish_canonical_compact(&projection);
        Ok(())
    }
    ...
    pub(crate) fn observe_token_count(
        ...
        let Some(usage) = event.info.as_ref().map(|info| &info.last_token_usage) else {
            return;
        };
    ...
    pub(crate) fn validated_trim_fact(
        &self,
        request: &spine_core::TrimRequest,
    ) -> Result<SpineOperationFact, CoordinatorError> {
        self.require_healthy()?;
        Ok(self.runtime.validated_trim_fact(request)?)
    }
