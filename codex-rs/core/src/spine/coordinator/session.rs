...
impl Session {
    ...
    pub(crate) async fn persist_install_spine_canonical_commit(
        ...
        self.replace_spine_context_items(installed.context.items.clone())
            .await;
        let mut coordinator = self.lock_spine_coordinator();
        let coordinator = coordinator
            .as_mut()
    ...
    pub(crate) fn validated_spine_trim_fact(
        &self,
        request: &spine_core::TrimRequest,
    ) -> Option<SpineOperationFact> {
        self.try_spine("build trim fact", |coordinator| {
            coordinator.validated_trim_fact(request)
        })
    }
