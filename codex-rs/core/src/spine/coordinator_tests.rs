...
fn open_source() -> [ResponseItem; 2] {
    [
        ResponseItem::FunctionCall {
            id: Some("open-request".to_string()),
            name: "open".to_string(),
...
fn spine_canonical_equivalence_preserves_ordinary_context() {
    ...
    let installed = coordinator
        .install_canonical_sampling(commit)
        .expect("install");
    let authoritative = super::derive_from_rollout(
        &expected
            .iter()
            .cloned()
    ...
    assert_eq!(installed.projection, authoritative.spine);
}
...
fn spine_canonical_equivalence_uses_explicit_fact_for_transition() {
    ...
    let installed = coordinator
        .install_canonical_sampling(commit)
        .expect("install");
    let mut rollout = vec![RolloutItem::ResponseItem(message("user", "question"))];
    rollout.extend(open_source().into_iter().map(RolloutItem::ResponseItem));
    let authoritative = super::derive_from_rollout(&rollout);
...
fn spine_sampling_commit_is_self_contained() {
    ...
    let installed = coordinator
        ...
        .expect("install");
    let mut rollout = vec![RolloutItem::ResponseItem(user)];
    rollout.extend(open_source().into_iter().map(RolloutItem::ResponseItem));
    let authoritative = super::derive_from_rollout(&rollout);
    assert_eq!(installed.projection, authoritative.spine);
    assert_eq!(installed.context.items, authoritative.context);
...
fn canonical_replay_accepts_host_tool_output_presentation_difference() {
    ...
    let request = ResponseItem::FunctionCall {
        id: Some("large-request".to_string()),
    ...
    let processed_output = ResponseItem::FunctionCallOutput {
        id: Some("large-output".to_string()),
        call_id: "large-call".to_string(),
    ...
    let raw_output = ResponseItem::FunctionCallOutput {
        id: Some("large-output".to_string()),
        call_id: "large-call".to_string(),
        output: FunctionCallOutputPayload::from_text(raw_body.clone()),
        internal_chat_message_metadata_passthrough: None,
    };
    let mut rollout = vec![
    ...
    let replayed = coordinator()
        ...
        .expect("replay canonical rollout");
    let mut expected_projection = expected.projection;
    let [spine_core::ContextItem::ToolCall(group)] =
        expected_projection.visible_context.as_mut_slice()
    else {
        panic!("expected one tool group");
    };
    group.calls[0].output = Some(raw_body);
...
#[test]
fn codex_context_materialization_runs_after_sdk_install() {
    let mut coordinator = coordinator();
    coordinator
        .observe_response_items(&[message("user", "question")])
        .expect("observe prompt source");
    ...
    coordinator
        .observe_response_items(&[
            ResponseItem::FunctionCall {
                id: Some("close-request".to_string()),
            ...
            ResponseItem::FunctionCallOutput {
                id: Some("close-output".to_string()),
                call_id: "close-call".to_string(),
                output: FunctionCallOutputPayload {
    ...
    let prepared = coordinator
        .prepare_canonical_sampling(close)
        .expect("SDK preparation must not depend on Codex item sizing");
    assert!(matches!(
        coordinator.install_canonical_sampling(prepared),
...
fn spine_execution_fact_commits_only_after_lifecycle_success() {
    ...
    assert_eq!(commit.projection, authoritative.spine);
}
